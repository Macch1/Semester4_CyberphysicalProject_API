namespace Semester4_CyberphysicalProject_API.DMI_Objects
{
    ...
    public class DMI_StationObservations_DataClass
    {
        ...
        public DMI_StationObservations_DataClass(string station_ID, List<DMI_Observation_DataClass> dmi_Observations)
        {
            ...
            // In case of problems with the Station IDs, we reject the construction of the data-class.
            // We want the data to be trustworthy, and not faulty.
            if (!isValid)
            {
                // Print the problem to the console for debugging purposes.
                Console.WriteLine($"[ERROR] DMI_StationObservations_DataClass: Observation mismatch detected. Expected all observations to belong to station ID '{station_ID}', but one or more observations had a different station ID.");
                ...
                throw new InvalidOperationException($"Cannot create DMI_StationObservations_DataClass: One or more observations do not belong to station ID '{station_ID}'. The data structure must be consistent.");
            }

            // All observations are valid — populate the internal observations list.
        ...
        /// <returns>True when the internal observations list has been successfully populated.</returns>
        ...
        /// Returns false immediately if any observation has a mismatching station ID.
        /// </summary>
        ...
        /// <returns>True if all observations match the station ID, false if any mismatch is found.</returns>
